<script>
	import * as _ from 'lamb';
	import Switch from '@svizzle/ui/src/Switch.svelte';

	import BarchartVDiv from 'app/components/svizzle/BarchartVDiv.svelte';
	import {MIN_SCORE} from 'app/config';
	import {_keyOrgTypeValueOrgsCount} from 'app/stores/data';
	import {
		_orgSearchText,
		_orgTypesSelectionMode,
		_selectedOrgTypes,
		orgTypesSelectionModes,
		toggleOrgType,
		toggleOrgTypesSelectionMode,
	} from 'app/stores/selection';
	import {_minScore} from 'app/stores/topics';

	let searchText = '';

	const toggledOrgType = ({detail: {id}}) => toggleOrgType(id);
	$: console.log($_orgSearchText);
</script>

<div class='Settings'>

	<!-- Search -->

	<div class='panel'>
		<h3>Org Search</h3>

		<div class='item'>
			<input type='text' bind:value={searchText} />
			<button on:click={() => $_orgSearchText = searchText}>
				Search
			</button>
		</div>
		<div class='item'>
			Selected orgs: 0
		</div>
	</div>
	
	<!-- Org types -->

	<div class='panel'>
		<h3>Org types</h3>

		<div class='item'>
			<BarchartVDiv
				isInteractive={true}
				items={$_keyOrgTypeValueOrgsCount}
				on:clicked={toggledOrgType}
				selectedKeys={$_selectedOrgTypes}
				theme={{
					padding: 0,
					selectedKeyBackgroundColor: 'cornsilk',
				}}
			/>
		</div>
		<div class='item'>
			<Switch
				value={$_orgTypesSelectionMode}
				values={orgTypesSelectionModes}
				on:toggled={toggleOrgTypesSelectionMode}
			/>
		</div>
	</div>

	<!-- Topics -->

	<div class='panel'>
		<h3>Topics</h3>

		<div class='item'>
			<label for='minScore'>Min score</label>
			<input
				bind:value={$_minScore}
				id='minScore'
				list='tickmarks'
				max=100
				min={MIN_SCORE}
				step=10
				type='range'
			>
			<datalist id='tickmarks'>
				{#each _.range(MIN_SCORE, 110, 10) as score}
					<option value={score}></option>
				{/each}
			</datalist>
		</div>
	</div>

</div>

<style>
	.Settings {
		width: 100%;
		height: 100%;
		padding: 1em;
	}

	.panel:not(:last-child) {
		border-bottom: 1px solid lightgrey;
		margin-bottom: 1em;
		padding-bottom: 1em;
	}

	.item {
		align-items: center;
		display: flex;
		gap: 1em;
		justify-content: center;
	}

	.item input {
		flex: 1;
	}
</style>
