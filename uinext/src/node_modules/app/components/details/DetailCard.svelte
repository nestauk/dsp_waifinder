<script>
	import Link from '@svizzle/ui/src/Link.svelte';
	import Icon from '@svizzle/ui/src/icons/Icon.svelte';
	import Link2 from '@svizzle/ui/src/icons/feather/Link2.svelte';
	import ChevronDown from '@svizzle/ui/src/icons/feather/ChevronDown.svelte';
	import ChevronUp from '@svizzle/ui/src/icons/feather/ChevronsUp.svelte';
	import Copy from '@svizzle/ui/src/icons/feather/Copy.svelte';
	
	import * as _ from 'lamb';

	import {_dataset} from 'app/stores/dataset';
	import {_focusedOrgId} from 'app/stores/interaction';
	import {_isSmallScreen} from 'app/stores/layout';
	import {
		asyncUpdateTopicDetails,
		clearActiveTopic
	} from 'app/stores/topics';
	import {getTopicLabel} from 'app/utils/dataUtils';
	import {getWikipediaURL} from 'app/utils/dbpedia';

	import Pill from './Pill.svelte';

	export let item;
	export let minScore = 60;

	let isFolded = true;
	let shouldFold = false;
	let isDescriptionUIVisible = false;

	const hideDescriptionUI = () => {
		isDescriptionUIVisible = false;
	}
	const showDescriptionUI = () => {
		isDescriptionUIVisible = true;
	}

	const copyText = text => text && navigator.clipboard.writeText(text);

	const activateFoldToggling = () => {
		shouldFold = true;
	}
	const cancelFoldToggling = () => {
		shouldFold = false;
	}
	const toggleFolded = () => {
		if (shouldFold) {
			isFolded = !isFolded;
		}
	}

	$: postCode = item?.location?.postcode || '';
	$: place = item?.place?.name || '';
	$: sep = (postCode && place) ? ' - ' : '';
	$: address = `${postCode}${sep}${place}`;
	$: topics = _.filter(item?.topics || [], ({score}) => score >= minScore);
	$: types = item?.types || [];
	$: isFocused = item.id === $_focusedOrgId;
</script>

<div
	class:highlighted={isFocused}
	class='DetailCard'
	on:mouseenter={() => _focusedOrgId.set(item.id)}
	on:mouseleave={() => _focusedOrgId.set(null)}
>

	<!-- row 1 -->

	<div class='flex between'>
		<div class='flex'>
			<span class='name'>{item?.name || ''}</span>
			{#if item?.url}
				<Link
					href={item.url}
					target='_blank'
				>
					<Icon
						glyph={Link2}
						size={20}
					/>
				</Link>
			{/if}
		</div>
		{#if isFocused}
			<div class='dot' />
		{/if}
	</div>

	<!-- row 2 -->

	<div
		on:mouseenter={showDescriptionUI}
		on:mouseleave={hideDescriptionUI}
	>
		{#if isDescriptionUIVisible}
			<div class='descriptionUI'>
				<span on:click={toggleFolded}>
					<Icon glyph={isFolded ? ChevronUp : ChevronDown} />
				</span>
				<span on:click={() => copyText(item?.description)}>
					<Icon glyph={Copy} />
				</span>
			</div>
		{/if}
	
		<p
			class='description'
			class:folded={isFolded}
			on:click={toggleFolded}
			on:mousedown={activateFoldToggling}
			on:mousemove={cancelFoldToggling}
		>
			{item?.description || ''}
		</p>
	</div>

	<!-- row 3 -->

	<div class='flex wrap'>
		{#if address !== ''}
			<div class='tag address'>
				<span>{address}</span>
			</div>
		{/if}
		{#each types as type}
			<div class='tag type'>
				<span>{type}</span>
			</div>
		{/each}
	</div>

	<!-- row 4 -->

	<div class='flex wrap topics'>
		{#if $_isSmallScreen}
			{#each topics as {id, score}}
				<div on:click={() => asyncUpdateTopicDetails(id)}>
					<Pill
						{score}
						label={getTopicLabel(id)}
					/>
				</div>
			{/each}
		{:else}
			{#each topics as {id, score}}
				<div class='topic'>
					<Link
						href={getWikipediaURL(id)}
						target='_blank'
					>
						<div
							on:mouseenter={() => asyncUpdateTopicDetails(id)}
							on:mouseleave={clearActiveTopic}
						>
							<Pill
								{score}
								label={getTopicLabel(id)}
							/>
						</div>
					</Link>
				</div>
			{/each}
		{/if}
	</div>
</div>

<style>
	.DetailCard {
		display: grid;
		gap: 0.8em;
		padding: 1em;
	}
	.highlighted {
		background-color: lavender;
	}

	.flex {
		align-items: center;
		display: flex;
		gap: 0.4rem;
		overflow-x: hidden;
	}
	.wrap {
		flex-wrap: wrap;
	}
	.between {
		justify-content: space-between;
	}

	/* row 1 */

	.flex {
		align-items: center;
		display: flex;
		gap: 0.4rem;
		overflow-x: hidden;
	}
	.wrap {
		flex-wrap: wrap;
	}

	.name {
		font-size: 1.2em;
	}

	.dot {
    width: 12px;
    height: 12px;
    background: red;
    -moz-border-radius: 50%;
    -webkit-border-radius: 50%;
    border-radius: 50%;
	}

	/* row 2 */

	.description {
		cursor: pointer;
	}
	.description.folded {
		/* These `-webkit` properties work in Firefox and Edge and Safari 15.4. */
		-webkit-box-orient: vertical;
		-webkit-line-clamp: 3;
		display: -webkit-box;
		overflow: hidden;
		text-overflow: ellipsis;
	}

	.descriptionUI {
		float: right;
		cursor: pointer;
	}
	/* row 3 */

	.tag span {
		font-style: italic;
		padding: 0.2em 0.5em;
	}
	.address span {
		background-color: aquamarine;
	}
	.type span {
		background-color: cornsilk;
	}

	/* row 4 */

	.topic {
		max-width: 100%;
	}
</style>
