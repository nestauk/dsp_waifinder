...
const makeIsOrgInPlaces = placesIds =>
	({place_id}) => placesIds.includes(place_id);

export const _placesSearchIds = derived(
	[
		_dataset,
		_placesSearchRegex
	],
	([
		...
		_.values, // TBD should we also keep a places array in memory?
		_.filterWith(({name}) => placesSearchRegex.test(name)),
		_.mapWith(_.getKey('id'))
	])(placesById));
...
export const _orgs = derived(
	...
	([
		...
		if (isRegexpNotEmpty(placesSearchRegex)) {
			predicates.push(makeIsOrgInPlaces(placesSearchIds))
		}
