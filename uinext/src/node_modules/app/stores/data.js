...
export const _orgs = derived(
	...
	([bboxOrgs, selectionMode, selectedOrgTypes, orgSearchText]) => {
		const predicates = [makeIsOrgOfTypes(selectedOrgTypes, selectionMode)];

		if (get(_orgSearchText) !== '') {
			predicates.push(makeIsTextInOrg(orgSearchText.toLowerCase()))
		}
...
export const _orgsCount = derived(
	[_orgs],
	([orgs]) => orgs.length);
