...
After this, another pipeline was run to add postcodes for the funders (since we only had the funder name). Details about [this script](/etl/bin/fillMissingLocations.js) can be found [here](etl/README.md). This generated the output file "../etl/data/outputs/ai_map_orgs_places_locations_populated.json". Finally, using this enhanced dataset we added the place information (e.g. the city name) for these funders and updated the output dataset.
